	...
	null,
{
	...
	registerViewWidget: function(oWidget){
		for (var i = 0; i < this.oWidgets.length; i++) {
			if (this.oWidgets[i].sId == oWidget.sId)
				return false;
		}
	...
	registerDataLoader: function(oDataLoader){
		for (var i=0; i<this.oDataLoaders.length; i++) {
			if ((this.oDataLoaders[i] == oDataLoader) || this.oDataLoaders[i].equals && this.oDataLoaders[i].equals(oDataLoader)) {
				var bFound = true;
				break;
			}
		}
...
//						this.oDataLoaders[i].load(oDate1, oDate2);
	...
	addEvent: function(oEvent, oCaller){
		...
		// continuous event could have duplicate UNIDs. (SPR KRAU86UDG3)
		if (sUnid && this.afAdded[sDate] && this.afAdded[sDate][sUnid] && !oEvent.fContinuous)
			return;
	...
	checkForConflicts: function(oStartDate){
		// return if there is no entry in that day
		var sDate = oStartDate.getISO8601String();
		if (!this.aoEventsByDate[sDate].length)
			return;

		// sort all events before checking conflicts
		this.aoEventsByDate[sDate].sort(function (oElem1, oElem2) {
			if (oElem1.bAllday)
				return -1;
			else if (oElem2.bAllday)
				return 1;
		...
		for (var i=0; i<this.aoEventsByDate[sDate].length; i++)
			this.aoEventsByDate[sDate][i].bConflicted = false;

		// check for conflicts
		for (var i=0, nGroup=0; i<this.aoEventsByDate[sDate].length; i++) {
			var oEvent1 = this.aoEventsByDate[sDate][i];
			oEvent1.nGroup = (nGroup = oEvent1.bConflicted? nGroup: i);
			for (var j=i+1; j<this.aoEventsByDate[sDate].length; j++) {
				var oEvent2 = this.aoEventsByDate[sDate][j];
				if (this.isConflicted(oEvent1, oEvent2)) {
					oEvent1.bConflicted = oEvent2.bConflicted = true;
					// check conflicts with the real data here again, and don't display conflict bars if entries are not conflicted
					// in the case that either entry is reminder.
					if(!oEvent1.hasEndTime || !oEvent2.hasEndTime) {
						// reminder has the same start and end time
						oEvent1.hasConflictBar = oEvent2.hasConflictBar = (oEvent1.oStartTime <= (oEvent2.hasEndTime? oEvent2.oEndTime: oEvent2.oStartTime) 
																			&& oEvent2.oStartTime <= (oEvent1.hasEndTime? oEvent1.oEndTime: oEvent1.oStartTime));
					} else 
						oEvent1.hasConflictBar = oEvent2.hasConflictBar = true;
				}
			}
		}
		...
		// optimize layout
		this.aoColumns = [];
		for (var i=0, nMaxIndex=1; i<this.aoEventsByDate[sDate].length; i++) {
			// find available column
			var oEvent1 = this.aoEventsByDate[sDate][i], oEvent2 = this.aoEventsByDate[sDate][i+1];
			for (var nIndex=0; nIndex<this.aoEventsByDate[sDate].length; nIndex++) {
				// insert the event to available column
				if (this.isColumnAvailable(nIndex, oEvent1)) {
					oEvent1.nIndexInConflicts = nIndex;
					this.aoColumns[nIndex] = this.aoColumns[nIndex].concat([oEvent1]);
					break;
				}
			}

			// number of columns need to display all conflict events
			nMaxIndex = oEvent1.bConflicted? Math.max(nMaxIndex, nIndex+1): 1;

			// set nMaxIndex to the group of events that are conflicted each other
			if (!oEvent2 || oEvent1.nGroup != oEvent2.nGroup) {
				for (var j=oEvent1.nGroup; j<=i; j++)
					this.aoEventsByDate[sDate][j].nConflicts = nMaxIndex;
				nMaxIndex = 1;
			}
		}

		// expand width if next column is available
		for (var i=0; i<this.aoEventsByDate[sDate].length; i++) {
			var oEvent = this.aoEventsByDate[sDate][i];
			oEvent.nWidthInConflicts = 1;
			if (oEvent.bConflicted)
				for (var nIndex=oEvent.nIndexInConflicts+1; nIndex<oEvent.nConflicts && this.isColumnAvailable(nIndex, oEvent); nIndex++, oEvent.nWidthInConflicts++) {}
		}
	},
	isConflicted: function(oEvent1, oEvent2) {
		if (oEvent1.bAllday || oEvent1.bHide || oEvent2.bAllday || oEvent2.bHide)
			return false;

		var oStart1 = oEvent1.oStartTime.getDate();
		var oEnd1 = oEvent1.oEndTime ? oEvent1.oEndTime.getDate(): null;
		if (oEnd1 == null) {
			var oEnd1 = new Date(oStart1);
			oEnd1.setTime(oEnd1.getTime() + 3600000);
		}
		var oStart2 = oEvent2.oStartTime.getDate();
		var oEnd2 = oEvent2.oEndTime? oEvent2.oEndTime.getDate(): null;
		if (oEnd2 == null) {
			var oEnd2 = new Date(oStart2);
			oEnd2.setTime(oEnd2.getTime() + 3600000);
		}
		return (oStart1 < oEnd2 && oStart2 < oEnd1);
	},
	isColumnAvailable: function(nIndex, oEvent) {
		if (!this.aoColumns[nIndex]) {
			this.aoColumns[nIndex] = [];
			return true;
		}
		for (var i=0; i<this.aoColumns[nIndex].length; i++) {
			if (this.isConflicted(oEvent, this.aoColumns[nIndex][i]))
				return false
		}
		return true;
	},
	...
	addEvents: function(store, result){
		...
		for (var i = 0; i < result.length; i++) {
			...
			var oEvent = {
				...
				hasEndTime: store.getValue(item, "hasEndTime"),
				_item: item,
				clone: function(){
					var oNewEvent = {};
					for (var s in this) {
			...
			// preserve original stard and end times
			oEvent.oDataStartTime = oEvent.oStartTime.clone();
			oEvent.oDataEndTime = oEvent.oEndTime.clone();
			...
			if (oEvent.oEndTime.getDate().getTime() < oListener.oStartTime.getDate().getTime()) {
				...
				if(!(oNextEvent.oEndTime.nHours == 0 && oNextEvent.oEndTime.nMinutes == 0 && oNextEvent.oEndTime.nMilliseconds == 0)) {
