	...
	null,
{
	...
	constructor: function(sId){
		...
		if(dojo.isIE){
			...
			dojo.toggleClass(this.oCover, 'dijitHidden', true);
		}
		this.aoActiveDrops = [];
		this.nOrient = 0;
		this.nCurrentLevel = 0;
		dojo.connect(dojo.doc.body, 'onclick', this, "hide");
		dojo.connect(dojo.doc.body, 'onkeyup', this, "handleKey");
	},
	...
	setPos: function(ev, oElem, fOppositeEdge, fCenter){
		...
		} else if(3 <= dojo.isFF && this.isRTL) {
	...
	hide: function(ev, nIndex){
		...
		if (has("mac") && (navigator.userAgent.match(/Mac|iPhone/i)) && ev && ev.ctrlKey)
			return;
...
//			return;
		...
		if (nIndex < 0) {
//			dojo.toggleClass(oElem, 's-hidden', true);
	
//			if(dojo.isFF){
				// SPR DYHG76H4XC - we have to hide the scrollbar first before we hide the menu, otherwise it will persist (Firefox/Mac)
//				dojo.toggleClass(oElem, 's-menu-overflow-hidden', false);
//			}
		}
