	...
	[dijit._Widget, dwa.common.dropdownBox],
{
	...
	activate: function(oDropdownManager) {
		...
		//TODO: need to check the work around for IE is not required.
	},
	...
	_toggleCheckMark: function(n) {
		...
		dojo.removeClass(isShown ? transparentElem : iconElem, "dijitHidden");
		dojo.addClass(isShown ? iconElem : transparentElem, "dijitHidden");
	...
	_isCheckMarkIconShown: function(n) {
		return dojo.hasClass(dojo.byId(this.sId + '-check-' + n + '-transparent'), "dijitHidden");
	},
	...
	_resetDisabledRows: function() {
		for (var n = 0; n < this.menuInfo.length - this._numOfSeparators; n++) {
			...
			var ariaLabel = isDisabled ? label + " unavailable" : (this.menuInfo[descriptorIndex].subMenu ? label + " sub menu" : label);
			if (8 <= dojo.isIE || 3 <= dojo.isFF) {
				dojo.byId(this.sId + '-item-' + n + '-label').innerHTML = ariaLabel;
			} else if (dojo.isIE <= 7 || dojo.isFF <= 2) {
				row.title = ariaLabel;
			}
		}
	},
	...
	draw: function() {
		var D_NoUserSelect = dojo.isFF ? '-moz-user-select:none;' : (dojo.isWebkit ? '-webkit-user-select:none;' : '');
		...
		for (var n = 0, i = 0; i < Math.max(aoNodes.length, this.menuInfo.length); i++, n += (!fSeparator - 0)) {
			...
			if (fSeparator) {
				asHtml[asHtml.length] = '<tr>'
				 ...
				 + '<img alt="" class="s-img-display" width="1" height="1" src="' + this._blankGif + '"/>'
				 + '</td>'
				 + '</tr>'
				 + '<tr>'
				...
				var sImg = '<span style="position:relative;display:inline-block;width:3px;height:' + (!this.isRTL ? 5 : 7) + 'px;overflow:hidden">'
					+ '<img alt="sub menu" class="s-img-display" src="' + dojo.moduleUrl("dwa.common", "/images/basicicons.gif") + '" style="top:-20px;left:-' + (!this.isRTL ? 50 : 120) + 'px;position:absolute;border-width:0px;"/>'
					+ '</span>';
				var sCheck;
				if (this.menuInfo[i].isChecked == undefined) {
					sCheck = '';
				} else {
					sCheck = '<img id="' + this.sId + '-check-' + n + '-transparent" src="' + this._blankGif + '" width="7" height="7" class="' + (this.menuInfo[i].isChecked ? 'dijitHidden' : 's-img-display') + '"/>'
					+ '<span style="position:relative;width:7px;height:7px;overflow:hidden"'
					+ ' class="' + (!this.menuInfo[i].isChecked ? 'dijitHidden' : 's-img-display') + '"'
					+ 'id="' + this.sId + '-check-' + n + '-icon">'
					+ '<img alt="check" align="center"  class="s-img-display" src="' + dojo.moduleUrl("dwa.common", "images/basicicons.gif")
				...
				if (asDescriptors.length > 1) {
					...
					var sIconImage = asIconDescriptor.length > 3 ?
					 this._blankGif : asIconDescriptor[2];
				...
				if (this.supportScreenReader) {
					...
					sAriaLabel = this.menuInfo[i].isDisabled ? sAriaLabel + " unavailable" : fChild ? sAriaLabel + " sub menu" : sAriaLabel;
				}
				asHtml[asHtml.length]
				 = '<tr id="' + this.sId + '-item-' + n + '" descriptorIndex="' + i + '"'
				 + ' class="' + (this.menuInfo[i].isDisabled ? this._disabledClass : '') + '" havechild="' + !!fChild + '" onmousedown="return false;"'
				 + '>'
				 + '<td id="' + this.sId + '-item-' + n + '-selection" class="' + sClass + (!this._isSelectionExists ? ' dijitHidden' : '') +  '"'
				 ...
				 + sCheck
				 + '</td>'
				 + '<td class="' + sClass + (!fIconExists ? ' dijitHidden' : '') +  '"'
				 + ' align="' + D_ALIGN_DEFAULT + '" unselectable="on" style="white-space:nowrap;overflow:hidden;' + D_NoUserSelect + '">'
				 + sIcon
				 + '</td>'
				 + '<td id="' + this.sId + '-td-' + n + '" tabindex="' + (this.handleFocus ? 0 : -1) + '" class="' + sClass + '" align="' + D_ALIGN_DEFAULT + '" unselectable="on" style="white-space:nowrap;' + D_NoUserSelect
				 + (dojo.hasClass(dojo.body(), "dijit_a11y") ? '"' : 'outline-width:0px;" hidefocus="true"')
				 + (8 <= dojo.isIE || 3 <= dojo.isFF ? ' aria-labelledby="' + this.sId + '-item-' + n + '-label"' : (dojo.isIE <= 7 || dojo.isFF <= 2 ? ' title="' + (this.supportScreenReader ? sAriaLabel : "") + '"' : ''))
				 + '>'
				 + sLabel
				 + '</td>'
		...
		this.domNode.innerHTML = '<table role="presentation" onmousedown="return false;" cellspacing="0" cellpadding="0" unselectable="on" style="' + D_NoUserSelect + 'border-width:0px;"><tbody unselectable="on" style="' + D_NoUserSelect + '">' + asHtml.join('') + '</tbody></table>' + asAriaLabel.join('');
	...
	setActive: function(nIndex, fShowChild) {
		...
		if (!dojo.hasClass(dojo.body(), "dijit_a11y")) {
			for (var n = 0; n < this.menuInfo.length - this._numOfSeparators; n++) {
				...
				if (checkMarkIcon && !dojo.hasClass(checkMarkIcon, "dijitHidden")) {
		...
		if (!this.handleFocus && dojo.hasClass(dojo.body(), "dijit_a11y")) {
			...
			for (var aoElems = this.domNode.getElementsByTagName('*'), i = 0; i < aoElems.length; i++) {
				...
				for (j = 0; j < asHighlightClass.length; j++) {
					dojo.toggleClass(aoElems[i], asHighlightClass[j], asMatch && asMatch[1] - 0 == nIndex);
				}
			}
		}
	},
	...
	showChild: function(nIndex, ev) {
		...
		if (dojo.isIE && dojo.doc.compatMode == "CSS1Compat") {
