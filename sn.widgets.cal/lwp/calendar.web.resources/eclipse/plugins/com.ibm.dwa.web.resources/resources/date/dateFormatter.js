	...
	null,
{
	constructor: function(nFormat){
		...
		this.oCalendarData.sDateFormat = this.oCalendarData.sDateFormat || this._msgs["D_DTFMT_DATE0"];
		this.oCalendarData.sDateSep = this.oCalendarData.sDateSep || this._msgs["D_DTFMT_DATESEP0"];
		this.oCalendarData.sDateFormatLong = this.oCalendarData.sDateFormatLong || this._msgs["D_DTFMT_FULLDATE0"];
		this.oCalendarData.sTimeFormat = this.oCalendarData.sTimeFormat || this._msgs["D_DTFMT_TIME0"];
		this.oCalendarData.sTimeSep = this.oCalendarData.sTimeSep || this._msgs["D_DTFMT_TIMESEP0"];
		if(this.oCalendarData.nFirstDayMonth == undefined){
			this.oCalendarData.nFirstDayMonth = this._msgs["D_CAL_DEFAULTFIRSTDAYMONTH"] - 0;
		}
		if(this.oCalendarData.nFirstDayFiveDay == undefined){
			this.oCalendarData.nFirstDayFiveDay = this._msgs["D_CAL_DEFAULTFIRSTDAYFIVEDAY"] - 0;
		}
		if(this.oCalendarData.nFirstDayWeek == undefined){
			this.oCalendarData.nFirstDayWeek = this._msgs["D_CAL_DEFAULTFIRSTDAYWEEK"] - 0;
		}
		...
		case 102:
			...
			break;
		case 103:
			this.sFormat = this._msgs['D_DTFMT_TIME5'];
			this.fTimeOnly = true;
			break;
		case 0:
		case 7:
		...
		case 18:
			...
			break;
		case 19:
			this.sFormat = dwa.common.utils.formatMessage(this._msgs["D_DTFMT_SHORTMONTH"], this.oCalendarData.f2DigitDay ? 'dd' : 'd');
			this.fDateOnly = true;
			break;
		case 20:
			this.sFormat = this.oCalendarData.sDateFormatLong.replace(/dddd/g, 'ddd').replace(/MMMM/g, 'MMM').replace(/\bdd\b/g, 'd');
			this.fDateOnly = true;
			break;
		case 21:
			this.sFormat = this._msgs["D_DTFMT_SHORTMONTH4YR_MY"];
			this.fDateOnly = true;
			break;
		case 22:
			this.sFormat = this._msgs["D_DTFMT_FULLSHORTDATETIME"];
			break;
		case 3:
		case 8:
		default:
	...
	omitYear: function(){
		this.sFormat = this.sFormat.replace(/-yyyy/, '').replace(/yyyy-/, '').replace(/, yyyy/, '').replace(/yyyy/, '')
		 .replace(/-yy/, '').replace(/yy-/, '').replace(/, yy/, '').replace(/yy/, '')
		 .replace(/-GGGG/, '').replace(/GGGG-/, '').replace(/GGGG/, '').replace(/-GGG/, '').replace(/GGG-/, '').replace(/GGG/, '')
		 .replace(/-gg/, '').replace(/gg-/, '').replace(/gg/, '').replace(/-g/, '').replace(/g-/, '').replace(/g/, '');
	},
	...
	oCalendarData: { // shared by each instance
