	...
	null,
{
	...
	isInvalid: function(){
		return isNaN(this.nYear + this.nMonth + this.nDate + this.nHours + this.nMinutes + this.nSeconds + this.nMilliseconds) || this.fDateOnly && this.fTimeOnly;
	},
	...
	getISO8601String: function(emitsep, noemitms){
		if (this.isInvalid())
			return '';
		...
		if (!this.fDateOnly) {
			...
			if(!noemitms){
				asDate[asDate.length] = ',' + (nMilliseconds < 10 ? '0' : '') + nMilliseconds;
			}
		}
	...
	getISO8601BasicFormatString: function(emitsep){
		if (this.isInvalid())
			return '';
	...
	null,
{
	...
	setRule: function(nOffset, nDSTBias, oDSTBoundaries, sZoneKey, sZoneName){
		...
		var nZoneID = dwa.common.utils.floorAbs(this.nOffset / 60) + (this.nOffset % 60) * 100;
		var sDSTRule = this.oDSTBoundaries ? this.oDSTBoundaries.getRuleStr() : void 0;
		var nOffsetHour = Math.floor(Math.abs(this.nOffset / 60));
		var nOffsetMinutes = Math.abs(this.nOffset) % 60;
